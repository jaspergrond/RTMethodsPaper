...
\acro{} was developed from method of \cite{Woods2015} which was also implemented in \textsc{Gasoline}.
...
explore how \acro{}'s adaptivity criteria control error and affect 
...
simulations. For a RT method to be these things it must remain effificient with 
...
The only ray-tracing code we are aware with similar scaling is TREERAY \citep{HaidEt18}.  TREERAY
...
\ref{fig:isosph}) we have also shown that \acro{}'s refinement criteria 
...
we can use any convenient timestep rather than being limited by the speed of light.
Directional 
...
However, in such a medium most sources contribute nothing to the local radiation field. 
...
of radiation with only a small constant multiplier added to the cost.  However, it 
