...
Starburst99, \cite{leithererEt99}).
\begin{figure}
...
back from large O-type stars before the first supernovae ($<1-3\rm{Myr}$) have a significant effect on star formation rate. Whereas \cite{daleEt12} 
conclude the effects on star formation rate to be small.

With this uncertainty in mind, it may seem surprising that RT has been treated 
...
$c$ in Equation \ref{eqn:classicrt}. \textit{Evolutionary} methods use a 
...
moment methods like OTVET \citep{gnedinAbel01} and RAMESE-RT 
...
to forward ray tracing. The main difference between forward and reverse ray 
...
density distribution is well sampled near the resolution element as opposed to 
...
tests ({eg. Figure 16 in \cite{rosdahlEt13}).

Photon packet propagation methods, such as TRAPHIC \citep{pawlikSchaye08}, 
employan evolutionary approach in which directional accuracy is easier to 
...
the prefactor on the scaling.  Nonetheless, moment methods such as TRAPHIC 
...
instantaneous methods that reverse raytracers alone do not solve. Therefore we 
have developed \acro{}, a $\bigO{N \log^2 N}$ reverse raytracer designed to 
...
general \acro{} algorithm (Subsection \ref{sec:algo}) as well as specifics of 
our implementation in \textsc{Gasoline} (Subsection \ref{sec:specs}). In 
...
I(s) = I(s_0)e^{-\tau(s)},
\end{equation}
where $I(s_0)$ is the intensity of the source and $\tau(s)$ is the
...
group of sources occupies the biggest tree cell that meets this criteria, all 
...
\end{equation}
The bounds of the integral are $R_0$, the size of a bucket 
...
\begin{equation}
\label{eqn:nssoln}
N_{\rm open} = \int_{\sqrt[3]{N_B/N}}^R  \frac{4\pi r^2}{\tO^3} dr
\sim \log{N/N_B}.
\end{equation}
...
be computed by computing the optical depth
...
efficient. Ideally, the criterion should be true when an average optical depth 
...
this reason we have chosen an $\tau$-based refinement criterion that is unique 
to \acro{}.
...
\label{fig:refine}
\end{figure}
Consider two rays through a large cell as in Figure \ref{fig:refine}. 
...
$\tau_{\rm max}$. We can then test the following refinement criterion
\begin{equation}
\label{eqn:refcrit}
...
If the refinement criterion passes at the bucket level, individual particles 
...
their neighbours. In the worst case where the refinement criterion passes the 
...
First let's consider the case where the refinement criterion always passes and 
...
\end{equation}
after sunstituting for $R_0$ using Equation \ref{eqn:ratio}. Since $\NS$ is 
also the number of rays computed, the total number of ray segments computed is 
just Equation \ref{eqn:nsint} multiplied by the number of ray segments
\begin{equation}
\label{eqn:nsegint}
N_{\rm seg} = \int_{\sqrt[3]{N_B/N}}^R 
\frac{4\pi r^2}{\tO^3}
...
originates from very large distances on the order of Gpc. This is too 
large a region to practically simulate, so radiation originating from 
...
only have to add temporary background star particles as the source of the 
background radiation. Also note that the simulation flux is over estimated 
near the central region and underestimated at large radii. When merging 
...
these trees is that the radiation tree is required to fill all space. This is 
in contrast to the main tree which ``squeezes'' cell bounds to the furthest 
extents of particles within the cell. This does not work in the context of RT 
as cell-ray intersections need to be computed to accumulate the total optical 
depth along the complete length of the ray.
...
enough, $N_{\rm B}=10$ was too small of a statistical sample to reliably 
compute cell density in this manner. This introduced errors of a fraction of a 
percent into the computed radiation field, noticeable as noise in uniform 
density distributions. To remedy this for particle based methods, we compute a 
cell density using the particle's SPH densities via 
\begin{equation}
...
the termination of the ray does not absorb photons. Otherwise, the front half 
...
we have created a novel IC comprised of a unit length glass of $N$ SPH gas and 
...
cost allows us to isolate the effects of the refinement criterion on cost. On 
...
resolution element, is plotted as a function of $N_{\rm 1D} = \sqrt[3]{N}$. 
...
which is typically the harder case to scale effectively, scales better than 
the weak scaling case. The strong scaling data turns over closer to the
$N\log^2{N}$ function and costs less than the weak scaling case. This is 
...
increased number of particles represents the same density and opacity 
distributions, the refinement criterion will operate in a similar way 
...
similarly to the no refinement data and fits $\bigO{N\log^2{N}}$ scaling 
behaviour. \comment{Hugh: ???} 
...
isolated by looking at particles in shadow. The four isothermal spheres have a 
...
$\rho_0=626$. The IC was made by adding SPH gas particles near the sphere 
centers using a stretched glass of. \comment{Grond: James, could you fill this 
part in? I don't have the script you used to do this on hand.} These 
...
more error at about 8\%. Decreasing the refinement criterion by an order of 
...
marginally above the $\rm{RMS}=\tr$ line. Again, but now in a more 
difficult test when compared to the sinusoidally perturbed glass IC, Equation 
\ref{eqn:reffrac} proves to be accurate in predicting that the maximum 
allowable error in flux per ray segment should be less than and proportional 
to $\tr$.Note that Equation \ref{eqn:reffrac} is technically only valid 
...
spheres test is an especially difficult test as there 
is only one source, and random errors cannot conspire to cancel as they do 
with many sources.

The isothermal spheres test is useful as it is representative of structure
commonly found in the universe and because it has an analytic solution. 
However, in SPH it is difficult to represent sharp density gradients with 
discrete resolution elements. This causes in-shadow particle errors relative 
to the analytic solution to be approximately an order of magnitude higher than 
...
and further from the source creating an ionization front. As the front moves 
radially outward from the source a radius is reached such that the ionization 
...
Again, \acro{} recreates these profiles quite well in a qualitative sense. 
However, \acro{} over-ionizes relative to all other solutions at every 
resolution. We believe this is not caused by the \acro{} algorithm 
specifically, as the isothermal test converged with resolution. It could 
possibly be a result of the ionization code and will be addressed in future 
work. \comment{Grond: Again, exactly what do we want to say here?}. On the 
other hand, the temperature profile lies in the middle of the scatter of the 
\cite{ilievEt06} solutions.
...
raycan be reduced, while maintaining a specified level of accuracy, to 
$\log N$.
\end{enumerate}
